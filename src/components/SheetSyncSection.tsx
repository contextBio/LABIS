...
/**
 * 구글시트 연동 — 관리자 설정 안의 한 구획.
 * 항목별 주소는 각 메뉴 상단에도 붙지만, 전체 목록·전체 실행·양식 안내는 여기가 본거지다.
 */
export async function SheetSyncSection({ labId }: { labId: number }) {
  ...
  try {
    const raw = await getLabSetting(labId, "sync_log");
  ...
  return (
    <>
      <SheetSources
        ...
        tabs={ITEM_TABS}
        from="/admin/settings"
        title="항목별 시트 주소 — 저장하면 그 자리에서 가져옵니다"
        defaultOpen
      />

      <Section title="구글시트 — 인증 상태">
      ...
      <Section title="구글시트 — 한 번에 실행">
        ...
        {log && (
          <ul className="mb-3 space-y-1 text-sm">
            <li className="text-xs text-slate-400">최근 실행 · {log.at}</li>
            {log.lines.map((line, i) => (
              <li
                key={i}
                className={line.startsWith("⚠") ? "text-amber-600" : line.startsWith("❌") ? "text-red-600" : ""}
              >
                {line}
              </li>
            ))}
        ...
        <p className="text-xs text-slate-400">
          가져오기 방식 — 과제·프로젝트·시료·실험·장비: 키(번호/코드/시리얼) 기준 <b>갱신/추가</b> ·{" "}
      ...
      <Section title="구글시트 — 시트 양식">
        ...
        <ul className="grid gap-1 text-xs text-slate-500 md:grid-cols-2">
          ...
          <li><b>프로젝트</b> — 프로젝트코드, 프로젝트명, 목표, 책임자, 연계과제번호, 시작일, 종료일, 상태, 비고</li>
